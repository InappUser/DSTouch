...
namespace DS4Api
{
    public class DS4
    {
        ...
        public DS4Data ReadDS4Data()
        {
           ...
           // bool tp_active3 = (data[8 + TOUCHPAD_DATA_OFFSET] >> 7) == 0; // > 2 touch detected
            ...
            //Debug.Log("1x:  Raw data(1): " + data[1 + TOUCHPAD_DATA_OFFSET] + " in bin: " + Convert.ToString(data[1 + TOUCHPAD_DATA_OFFSET], 2)
            //            + "Raw data(2): " + data[2 + TOUCHPAD_DATA_OFFSET] + " in bin: " + Convert.ToString(data[2 + TOUCHPAD_DATA_OFFSET], 2)
            //            + "or data "+ Convert.ToString((data[2 + TOUCHPAD_DATA_OFFSET]),2)
            //            + " masked: "+ Convert.ToString( data[2 + TOUCHPAD_DATA_OFFSET] & 0xF,2)
            //            +" shifted: "+Convert.ToString((data[2 + TOUCHPAD_DATA_OFFSET] & 0xF) << 8,2)
            //            + " To OR: " + Convert.ToString(data[1 + TOUCHPAD_DATA_OFFSET], 2)
            //            + " ORed: " + Convert.ToString((data[1 + TOUCHPAD_DATA_OFFSET]) | (data[2 + TOUCHPAD_DATA_OFFSET] & 0xF) << 8, 2)
            //            + " dec: " + Convert.ToString((data[1 + TOUCHPAD_DATA_OFFSET]) | (data[2 + TOUCHPAD_DATA_OFFSET] & 0xF) << 8, 10)
            //            +" end int "+touch1_x);
            //int touch3_x = data[9 + TOUCHPAD_DATA_OFFSET] | ((data[10 + TOUCHPAD_DATA_OFFSET] & 0xF) << 8);
            //int touch3_y = ((data[10 + TOUCHPAD_DATA_OFFSET] & 0xF0) >> 4) | ((data[11 + TOUCHPAD_DATA_OFFSET] << 4));
            //Debug.Log("1x"+ touch1_x + "1y" + touch1_y );

            //if (tp_active1 && tp_active2 && tp_active3)
            //    cState.Touches = new int[,] { {touch1_x, touch1_y}, { touch2_x, touch2_y }, { touch3_x, touch3_y } };
           ...
           // Debug.Log("1x" + cState.Touches[0,0] +"  "+touch1_x);
